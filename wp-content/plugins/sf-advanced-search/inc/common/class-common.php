...
namespace SF_Advanced_Search\Inc\Common;
...
//						'terms' => ! empty( $terms ) ? $terms : false,
	...
	public function advanced_search_form_markup( $form ) {
		...
		ob_start();
		include_once( 'views/html-sf-advanced-search-form.php' );
	...
	public function register_shortcodes() {
		...
		add_shortcode( 'sf-advanced-search', array( $this, 'shortcode_sf_advanced_search' ) );
	 ...
	 */
	public function shortcode_sf_advanced_search( $atts, $content = null ) {
		/*
		...
		if ( isset( $search_term ) && ! empty( $search_term ) ) {
			...
			ob_start();
			include_once( 'views/html-sf-advanced-search-results.php' );
	...
	/**
	 * Returns the search results if the form is submitted. Loads search results via ajax.
	 * The method consists partly of the same code as in 'shortcode_sf_advanced_search()'
	 * 	 
	 * Callback for the "wp_ajax_sf_advanced_search_form_response" and
	 * "wp_ajax_nopriv_sf_advanced_search_form_response" hooks in "class-init.php"	 
	 *
	 * @since    1.0.0
	 */
	public function advanced_search_form_response_handler() {
		
		// form security, verify nonce field
		if( isset( $_POST['sf_advanced_search_nonce'] ) && wp_verify_nonce( $_POST['sf_advanced_search_nonce'], 'sf_advanced_search_form_nonce') ) {
